...
const navigationItems = {
  team:      { title: 'Team',      content: teamTemplate() },
  project:   { title: 'Projects',  content: projectTemplate() },
  marketing: { title: 'Marketing', content: marketingTemplate() }
}
    ...
    <div class="col-md-4 col-lg-3 navigation-list">
      <div class="list-group" id="navigation-list-tab" role="tablist">
        ${Object.keys(navigationItems).map((navigationItem, i) => `
          <button class="list-group-item list-group-item-action ${i === 0 ? 'active' : ''}" id="navigation-list-${navigationItem}-list" data-bs-toggle="list" href="#navigation-list-${navigationItem}" role="tab" aria-controls="navigation-list-${navigationItem}">${Object.values(navigationItems)[i].title}</button>
        `).join('')}
        </div>
    </div>
